...
public class CustomerServiceController {
	 ...
	 *  Functionality:This method adds a new customer to DB by taking the Customer info 
	 *  Input:Customer
	 */
	...
	public ResponseEntity<Optional<CustomerModel>> getCustomer(@Valid @PathVariable("id") Long id) {
	...
	public ResponseEntity<List<Optional<CustomerModel>>> getAllCustomers() {
		...
		if (customerdtoList.isEmpty()) {
			log.info(
					"form CustomerServiceController: getAllCustomers method- no customer info is avaiable to retrieve");
	 ...
	 * Input:Customer first name and last name
	 */
	...
	public ResponseEntity<List<Optional<CustomerModel>>> searchCustomerNameBy(@PathVariable("fname") String fname,
		...
		List<Optional<CustomerModel>> customerNameList;
		customerNameList = customerServiceImpl.getCustomerByFnameAndLname(fname, lname);
		if (customerNameList.isEmpty()) {
			customerNameList = customerServiceImpl.getCustomerByFnameOrLname(fname, lname);
			if (customerNameList.isEmpty()) {
				log.info(
						"form CustomerServiceController: searchCustomerNameBy customerdtoOrList method- No of customer info is avaiable for given name");
				return new ResponseEntity<List<Optional<CustomerModel>>>(customerNameList, HttpStatus.NO_CONTENT);
			}
		}
		log.info(
				"form CustomerServiceController: searchCustomerNameBy customerNameList method- customer info for given name="
						+ customerNameList);
		return new ResponseEntity<List<Optional<CustomerModel>>>(customerNameList, HttpStatus.OK);
	}

	/*
	 * Method name: updateLivingAddress 
	 * Aim: to update customer address to DB
	 * Functionality:This method updates the customer address based on the customer id
	 * Input:Customer Id and address
	 */
	...
	public ResponseEntity<String> updateLivingAddress(@Valid @PathVariable("id") Long custId,
		...
		if (customerdto.isPresent()) {
			...
			return new ResponseEntity<String>(CustomerServiceConstants.CUSTOMER_ADDERESS_UPDATE_SUCCESS,
					HttpStatus.ACCEPTED);
		}
		log.info("LivingAddress is not updated");
		return new ResponseEntity<String>(CustomerServiceConstants.CUSTOMER_ADDERESS_UPDATE_FAILED,
